...
% EJ comments
% this script assumes that you are in a folder with the data files - it
% does not look for subfolders, which means that if the data live in
% folders per stack, we need to loop through the stacks

clear all;

%%
%This section defines parameters.
% add paths to where storm analysis code lives
addpath(genpath('C:\Users\Elina\storm-analysis'));
addpath(genpath('C:\Users\Elina\MERFISH_analysis'));
addpath(genpath('C:\Users\Elina\export_fig'));

% define some experiment details
dataPath            = fullfile('D:','processed');
experimentName      = '180712 Merfish';
resultsFolderName   = '001';                                % name of the round of storm analysis we want to look at
thisDataPath        = fullfile(dataPath,experimentName);

stackList   = dir(thisDataPath);
nStacks     = length(stackList);      % the first 2 outcomes in the directory are hidden folders we don't care about

resultsPath         = fullfile('D:','MERFISH results',experimentName,resultsFolderName);
analysisBasePath    = fullfile('D:','MERFISH results',experimentName,resultsFolderName,'FiducialReports');

nHybs       = 16;       % The number of hybridization/imaging rounds
maxFidDist  = 20;       % Maximum distance for fiducial tracking
refFrame    = 1;        % Reference frame for fiducial markers
maxSpotDist = 5;        % Distance between spots in different rounds
...
geneList = {};      
% % list of genes we want to extract and save the corrected XY coordinates for
% % if left empty, the default is all
% geneList{1} = 'SST';
% geneList{2} = 'GAD1';
% geneList{3} = 'GAD4';
...
parameters.imageTag             = imTag          % Initial name--typically describes the scope
parameters.imageMListType       = bitTag   % Tag for molecule list file for found RNA spots
parameters.fiducialMListType    = fidTag  % Tag for molecule list file for found fiducial spots
...
parameters.numHybs = nHybs                % The number of hybridization/imaging rounds
%parameters.bitOrder = fliplr(1:16);   % The order in which bits are imaged. The example was run in reverse
parameters.bitOrder = 1:nHybs %canonical order
...
codebookName    = 'merfish_codebook.fasta';
parameters.codebookPath = fullfile('D:','CodeBooks',codebookName);
...
defaults(end+1,:) = {'imageTag', 'string', imTag};            % Base tag for all images
defaults(end+1,:) = {'imageMListType', 'string', bitTag};     % Flag for image mlist
defaults(end+1,:) = {'fiducialMListType', 'string', fidTag};  % Flag for fiducial mlists
...
defaults(end+1,:) = {'numHybs', 'nonnegative', nHybs};         % Number of hybridizations
defaults(end+1,:) = {'bitOrder', 'boolean', 1:nHybs};          % Order of bits

% Parameters for fiducial tracking
defaults(end+1,:) = {'maxD', 'nonnegative', maxFidDist};     % Maximum distance for fiducial tracking
defaults(end+1,:) = {'fiducialFrame', 'nonnegative', refFrame};     % Reference frame for fiducial markers
defaults(end+1,:) = {'fiducialWarp2Hyb1', 'boolean', false};

% Parameters for constructing words from spots
defaults(end+1,:) = {'maxDtoCentroid', 'nonnegative', maxSpotDist};   % Distance between spots in different rounds
...
for z = 4:nStacks % the first 2 outcomes in the directory are hidden folders we don't care about, then the first two stacks we want to ignore as well cause they're poor quality duplicates
    
    thisStackPath   = fullfile(thisDataPath, stackList(z).name, resultsFolderName);
    if parameters.printedUpdates
        display('--------------------------------------------------------------');
        display(['Finding cells in ' thisStackPath]);
    end
    
    foundFiles = BuildImageDataStructures(thisStackPath, 'parameters', parameters)
    ...
    if parameters.printedUpdates
        display('--------------------------------------------------------------');
        display(['Analyzing data for cell ' num2str(numCells)]);
    end
...
%     end
    ...
    %% save results
    filename = strcat(stackList(z).name,'_merfishResults.mat');
    save(fullfile(resultsPath,filename),'words','parameters');
    
    %% extract XY coordinates for the genes & save them separately under a folder for each gene
    if isempty(geneList)
        geneList = unique({words.geneName});
        if isempty(geneList{1})             % some fields in word.geneName are empty if there was no match with any of the gene codes, and we want to ignore those
            geneList = geneList(~cellfun('isempty',geneList));
        end
    end
    
    extractGeneCoordinates_perTile(words,geneList);
