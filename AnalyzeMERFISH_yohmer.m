...
exampleDataPath = '/Users/yohisogai/Dropbox (UCL - SWC)/data/'
dataFolderName = 'merfish_run2_image63'
...
defaults(end+1,:) = {'maxD', 'nonnegative', 30};             % Maximum distance for fiducial tracking
defaults(end+1,:) = {'fiducialFrame', 'nonnegative', 1};    % Reference frame for fiducial markers
defaults(end+1,:) = {'fiducialWarp2Hyb1', 'boolean', true};

% Parameters for constructing words from spots
defaults(end+1,:) = {'maxDtoCentroid', 'nonnegative', 3};   % Distance between spots in different rounds
...
%words, which means that there were something wrong.
 ...
 %% Instead I will generate a table by gene, average xc and yc.
  ...
  figure
  hold on
  scatter([geneCoordinateList.xc], [geneCoordinateList.yc],40,'MarkerEdgeColor','cyan')
  
  %% save as transparent png
  
  exportPath = '/Users/yohisogai/data/';
  exportName = 'export000.png';
  filename1 = [exportPath exportName];
  
  figure;
  
  firstimage = imread('bit1.tif');
  B = imrotate(firstimage,0);
  imshow(B,[0,4000]);
  ...
  for j = 1:length(exactMatchWords)
    ...
    scatter([geneCoordinateList.xc], [geneCoordinateList.yc],30,'filled','green')
    scatter([imageData(2).mList.x], [imageData(2).mList.y],30,'filled','yellow')
  
  set(gca, 'Color', 'none'); % Sets axes background

  export_fig filename1 -transparent
